...
def confirm():
    try:
        ...
        if request.method == 'POST': 
            ...
            db = get_db()
            attempt = db.execute(
                'UPDATE forgotlink SET validuntil=?', (authid, utils.F_ACTIVE)
            ).fetchone()
            
            if attempt is not None:
                db.execute(
                    'UPDATE forgotlink SET id=?', (utils.F_INACTIVE, attempt['id'])
                )
                salt = hex(random.getrandbits(128))[2:]
                hashP = generate_password_hash(password + salt)   
                db.execute(
                    'UPDATE forgotlink SET userid=?', (hashP, salt, attempt['userid'])
                )
                db.commit()
                return redirect(url_for('auth.login'))
            else:
...
def change():
    try:
        ...
        if request.method == 'POST': 
            ...
            db = get_db()
            attempt = db.execute(
                'UPDATE forgotlink SET validuntil=?', (number, utils.F_ACTIVE)
            ).fetchone()
...
def forgot():
    try:
        ...
        if request.method == 'POST':
            ...
            db = get_db()
            user = db.execute(
                'SELECT email FROM forgotlink WHERE id=?', (email,)
            ).fetchone()
